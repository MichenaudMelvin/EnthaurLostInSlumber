...
void AENTAIPath::OnConstruction(const FTransform& Transform)
{
...
void AENTAIPath::UpdatePoints(bool bInConstructionScript)
{
#if WITH_EDITORONLY_DATA
	Arrows.Empty();
#endif
...
FVector AENTAIPath::GetDirection() const
{
...
FVector AENTAIPath::GetPointLocation(int8 PointIndex, float PawnHeight) const
{
...
bool AENTAIPath::GetTracedPointLocation(int8 PointIndex, FHitResult& HitResult)
{
...
bool AENTAIPath::IsOnFloor() const
{
	return Direction == EAxis::Z && bInvertDirection;
}

#if WITH_EDITORONLY_DATA
bool AENTAIPath::AttachAI(APawn* AI)
{
	if (!AI)
	{
		return false;
...
void AENTAIPath::DetachAI(APawn* AI)
{
	if (!AI)
	{
		return;
